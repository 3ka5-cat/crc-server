...
static int create_and_bind(long port)
{
	...
	for (rp = result; rp != NULL; rp = rp->ai_next) {
		...
		//close(sfd); // strange line from linux man oO
	}
...
int worker(int ind)
{
	...
	while (1) {					
		...
		count = read(clients[ind].fd, buf, 
							sizeof buf);
		//printf("read: %d\n", count);
		// TODO: check this two if. they are  possibly redundant
		if (count == -1) {
		/* If errno == EAGAIN, that means we have read all
			...
			fprintf(stdout, " Print checksum: %s from %d\n", 
				clients[ind].files[file_ind].file_name, clients[ind].fd);			
			memset(buf, 0, sizeof buf);
			count = sprintf(buf, "%lX", clients[ind].files[file_ind].CRC);
			buf[count] = 0xD;
			buf[count + 1] = 0xA;
			...
			if (res == -1 && errno == ENOENT) {				
				...
				strncpy(clients[ind].files[file_ind].file_name, buf + 1, 255);
